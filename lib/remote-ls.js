...
/**
 * Get the dependencies tree of a remote npm package
 */
function RemoteLS (opts) {
  if(!(this instanceof RemoteLS)) return new RemoteLS(opts)
...
RemoteLS.prototype._loadPackageJson = function (task, done) {
  ...
  request.get(this.registry().replace(/\/$/, '') + '/' + task.name, {json: true},
...
RemoteLS.prototype._guessVersion = function (versionString, packageJson) {
  ...
  var range = new semver.Range(versionString, true)

  var version = semver.maxSatisfying(Object.keys(packageJson.versions), range, true)
